...
package com.lylib.touch.objects
{
	...
	{	
		 ...
		 * 是否有惯性
		 */		
		private var _noInertia:Boolean= false;
		...
		private var _moCaLi:Number = 1.2;//1.2
		private var _timeID:int;
		public var isLr:Boolean;
		public var isInertiaEnd:Boolean;
		...
		private function onTouchBegin(e:TouchEvent):void
		{
			...
			{//trace("執行了")
				...
				_startScaleX = this.scaleX;
				_startScaleY = this.scaleY;
			}
			if(touchBeginFun){
				touchBeginFun();
			}
...
//			}
		...
		//	catch(error:Error) 
		//	{
			//	trace("touchMove RotatableScalable 205");
			//}
...
//			var hieBoo:Boolean = HitTest.complexHitTestObject(this, ApplicationData.getInstance().hitSp);
		
			if(topBianJie>1075||bottomBianJie<5)
			{
				ToolKit.log("卡片从上下出舞台了");
				isLr = true;
				isInertiaEnd = true;
				InertiaEnd();
			}else if(leftBianJie>1915||rightBianJie<5){
				ToolKit.log("卡片从左右抛出舞台了"+isLr);
				isLr = false;
				isInertiaEnd = true;
				InertiaEnd();
			}else{
		 ...
		 * 判断边界 
		 */		
		private function InertiaEnd():void
		{
			if(touchEndFun)
			{
				touchEndFun();
...
//					trace("删除后",_points.length);
			...
			if(_touchIDs.length==0){
				if(_noInertia==true)
				{
					if(this.scaleX!=_startScaleX||this.scaleY!=_startScaleY)
					{
						_timer.stop();
						isInertiaEnd = false;
						if(touchEndFun){
		...
		private function weiYi():void
		{
			...
			//trace(_upY,_upX,"weiYi",_startX,_startY);
			//触控结束的回调函数
			if(Math.abs(_upX-_startX)>20||Math.abs(_upY-_startY)>20)
			{//trace("enterFrame")
			//	trace("執行le慣性");
				_timer.stop();
		...
		public function removeEventListener1():void
		{
			this.removeEventListener(TouchEvent.TOUCH_BEGIN, onTouchBegin);
			this.removeEventListener(TouchEvent.TOUCH_MOVE, onTouchMove);
			this.removeEventListener(TouchEvent.TOUCH_ROLL_OUT, onTouchRollOut);
...
//			trace("Rotatable Dispose")
		...
		public function get noInertia():Boolean
		{
			return _noInertia;
		}

		public function set noInertia(value:Boolean):void
		{
			_noInertia = value;
		}
